...
func TestSingleSpecConfigureAllocatesPortsWhenMissing(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("configure zookeeper single: %v", err)
	}

	keys := []string{
		runtimecompose.ValueClientPort,
		runtimecompose.ValueFollowerPort,
		runtimecompose.ValueElectionPort,
	}
...
func TestClusterSpecConfigureAllocatesPortsWhenMissing(t *testing.T) {
	...
	spec := NewClusterSpec()
	service, err := spec.Configure(tpl.ServiceInput{
		Name:       "zookeeper-cluster-auto",
		Middleware: middlewareName,
		Template:   clusterTemplate,
	}, 1)
	if err != nil {
		t.Fatalf("configure zookeeper cluster: %v", err)
	}

	keys := []string{
		runtimecompose.ValueZK1ClientPort,
		runtimecompose.ValueZK2ClientPort,
		runtimecompose.ValueZK3ClientPort,
	}
	seen := make(map[int]struct{}, len(keys))
	for _, key := range keys {
		port, err := normalizePort(service.Values[key])
...
func TestClusterSpecConfigureRejectsDuplicateExplicitPort(t *testing.T) {
	...
	spec := NewClusterSpec()
	_, err := spec.Configure(tpl.ServiceInput{
		Name:       "zookeeper-cluster-fixed",
		Middleware: middlewareName,
		Template:   clusterTemplate,
		Values: map[string]any{
			runtimecompose.ValueZK1ClientPort: 2181,
			runtimecompose.ValueZK2ClientPort: 2181,
		},
	}, 1)
	if err == nil {
		t.Fatal("expected duplicate explicit zookeeper cluster port to be rejected")
	}
}
